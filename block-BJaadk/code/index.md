...
let doubleNum = numbers.forEach(double);
```
it is not return us any thing because we can not return 
```js
let numbers = [100, 200, 33, 45, 65, 76, 51];
function double(num) {
  console.log(num);
...
```
we can store the output in veriable after storing we can console the veriable the it is give us output
```js
let numbers = [100, 200, 33, 45, 65, 76, 51];
function addTwo(num) {
  return num + 2;
...
let doubleNumbers = numbers.map(addTwo);
```
when do we console then it is give us output 
```js
let numbers = [100, 200, 33, 45, 65, 76, 51];
function addTwo(num) {
  alert(num);
